...
APP_PASSWORD = "kxtz qslz vjzk euqs"
RECIPIENT_EMAIL = "hoeo456@g.skku.edu"
...
def check_keyword_similarity(notice_text, keywords):
    ...
    for i, keyword_embedding in enumerate(keyword_embeddings):
        similarity = float(notice_embedding @ keyword_embedding) / (
            np.linalg.norm(notice_embedding) * np.linalg.norm(keyword_embedding)
        )
...
def send_email_alert(notice_data, matched_keywords):
    try:
        ...
        keyword_text = ', '.join([f"{kw} score : ({score})" for kw, score in matched_keywords])
...
if __name__ == "__main__":
    # 이메일 주소 입력받기
    RECIPIENT_EMAIL = input("알림 받을 메일 주소 : ")
